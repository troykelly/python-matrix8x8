# hdmimatrix: An interface for IP control of generic 8x8 HDMI Matrix's

Allowing remote control of generic "Made in USA" (they are not) black
HDMI Matrix's
...
```
sudo easy_install hdmimatrix
```
or
```
pip install hdmimatrix
```

### Manually

hdmimatrix consists of a single file (hdmimatrix.py) that you can put in your python search path or in site-packages (or dist-packages depending on the platform)
...
hdmimatrix works with Python 2.6.x, 2.7.x and 3.x
...
## Using hdmimatrix with Max/MSP via Jython

You can use the hdmimatrix library within [Max/MSP](http://www.cycling74.com) by using [Nick Rothwell's](http://www.cassiel.com) Jython objects.  He recently updated the version to support Jython 2.7 which is required for hdmimatrix to work.

Download it here: https://github.com/cassiel/net.loadbang.jython

## Using hdmimatrix on iOS via Pythonista

You can use hdmimatrix on your iOS device via the [Pythonista](http://omz-software.com/pythonista) app.
