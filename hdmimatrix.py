...
'''
hdmimatrix by Troy Kelly - A HDMI Matrix library for Home Assistant
https://github.com/troykelly/python-matrix8x8
...
logger = logging.getLogger('hdmimatrix')
...
class HDMIMatrixException(Exception):
...
class HDMIMatrixRegistrationException(HDMIMatrixException):
    pass


class HDMIMatrixRequestTimeout(HDMIMatrixException):
    pass
...
class Light(object):
    ...
    """
    def __init__(self, matrix, light_id):
        self.matrix = matrix
    ...
    # Wrapper functions for get/set through the matrix, adding support for
    # remembering the transitiontime parameter if the user has set it
    def _get(self, *args, **kwargs):
        return self.matrix.get_light(self.light_id, *args, **kwargs)
    ...
    def _set(self, *args, **kwargs):
        ...
        return self.matrix.set_light(self.light_id, *args, **kwargs)
    ...
    def name(self, value):
        ...
        self.matrix.lights_by_name[self.name] = self
        del self.matrix.lights_by_name[old_name]
...
class SensorState(dict):
    def __init__(self, matrix, sensor_id):
        self._matrix = matrix
        self._sensor_id = sensor_id

    def __setitem__(self, key, value):
        dict.__setitem__(self, key, value)
        self._matrix.set_sensor_state(self._sensor_id, self)
...
class SensorConfig(dict):
    ...
    def __setitem__(self, key, value):
        dict.__setitem__(self, key, value)
        self._matrix.set_sensor_config(self._sensor_id, self)
...
class Sensor(object):
    ...
    def __init__(self, matrix, sensor_id):
        ...
        self._state = SensorState(matrix, sensor_id)
        self._config = {}
        self._recycle = None
    ...
    # Wrapper functions for get/set through the matrix
    def _get(self, *args, **kwargs):
        return self.matrix.get_sensor(self.sensor_id, *args, **kwargs)

    def _set(self, *args, **kwargs):
        return self.matrix.set_sensor(self.sensor_id, *args, **kwargs)
    ...
    def name(self, value):
        ...
        self.matrix.sensors_by_name[self.name] = self
        del self.matrix.sensors_by_name[old_name]
...
class Group(Light):
    ...
    """ A group of HDMI lights, tracked as a group on the matrix
    ...
    def __init__(self, matrix, group_id):
        Light.__init__(self, matrix, None)
    ...
    # remembering the transitiontime parameter if the user has set it
    def _get(self, *args, **kwargs):
        return self.matrix.get_group(self.group_id, *args, **kwargs)
    ...
    def _set(self, *args, **kwargs):
        ...
        if self.transitiontime is not None:
            ...
            if (args[0] == 'on' and args[1] is False) or (
                    kwargs.get('on', True) is False):
                self._reset_bri_after_on = True
        return self.matrix.set_group(self.group_id, *args, **kwargs)
    ...
    def lights(self):
        ...
        # response = self.matrix.request('GET', '/api/{0}/groups/{1}'.format(self.matrix.username, self.group_id))
        # return [Light(self.matrix, int(l)) for l in response['lights']]
        return [Light(self.matrix, int(l)) for l in self._get('lights')]
...
class AllLights(Group):
    ...
    """ All the HDMI lights connected to your matrix
...
class Matrix(object):
    ...
    """ Interface to the HDMI matrix

    You can obtain Output objects by calling the get_output_objects method:

        >>> b = Matrix(hostname='10.7.3.220')
        >>> b.get_output_objects()
        [<hdmimatrix.Output at 0x00>,
         <hdmimatrix.Output at 0x01>]

    Or more succinctly just by accessing this Matrix object as a list or dict:
    ...
    """
    def __init__(self, hostname=None, port=8000, config_file_path=None):
        """ Initialization function.

        Parameters:
        ------------
        hostname : string
            Hostname or IP address as dotted quad
        ...
        if config_file_path is not None:
            ...
            self.config_file_path = os.path.join(os.getenv(USER_HOME), '.python_hdmimatrix')
        elif 'iPad' in platform.machine() or 'iPhone' in platform.machine() or 'iPad' in platform.machine():
            self.config_file_path = os.path.join(os.getenv(USER_HOME), 'Documents', '.python_hdmimatrix')
        else:
            self.config_file_path = os.path.join(os.getcwd(), '.python_hdmimatrix')
    ...
    @property
    def name(self):
        '''Get or set the name of the matrix [string]'''
    ...
    def request(self, mode='GET', address=None, data=None):
        ...
        try:
            ...
            raise HDMIMatrixRequestTimeout(None, error)
    ...
    def get_ip_address(self, set_result=False):
        ...
        """ Get the matrix ip address from the meethue.com nupnp api """
    ...
    def register_app(self):
        """ Register this computer with the HDMI matrix hardware and save the resulting access token """
        registration_request = {"devicetype": "python_hdmimatrix"}
            ...
            for key in line:
                if 'success' in key:
                    with open(self.config_file_path, 'w') as f:
                        ...
                        logger.info('Reconnecting to the matrix')
                    self.connect()
                if 'error' in key:
                    error_type = line['error']['type']
                    if error_type == 101:
                        raise HDMIMatrixRegistrationException(error_type,
                                                        'The link button has not been pressed in the last 30 seconds.')
                    if error_type == 7:
                        raise HDMIMatrixException(error_type,
                                            'Unknown username')

    def connect(self):
        """ Connect to the HDMI matrix """
        logger.info('Attempting to connect to the matrix...')
        ...
        if self.ip is None or self.username is None:
            try:
                ...
                logger.info(
                    'Error opening config file, will attempt matrix registration')
                self.register_app()
    ...
    def __getitem__(self, key):
        """ Lights are accessibly by indexing the matrix either with
    ...
    def create_sensor(self, name, modelid, swversion, sensor_type, uniqueid, manufacturername, state={}, config={}, recycle=False):
        """ Create a new sensor in the matrix. Returns (ID,None) of the new sensor or (None,message) if creation failed. """
        data = {
            "name": name,
            "modelid": modelid,
            "swversion": swversion,
    ...
    def run_scene(self, group_name, scene_name, transition_time=4):
        ...
        As of 1.11 of the HDMI API the scenes are accessable in the
...
if __name__ == '__main__':
    ...
    while True:
        try:
            b = Matrix(args.host, config_file_path=args.config_file_path)
            break
        except HDMIMatrixRegistrationException as e:
            if PY3K:
                input('Press button on Matrix then hit Enter to try again')
            else:
                raw_input('Press button on Matrix then hit Enter to try again')  # noqa
